...
public class Monitor
{
	...
	private int pepper=2;//numbers of pepper shaker
	 ...
	 * -------------------------------
	 */
	public synchronized int availableShaker() {
		return pepper;
	}
	public synchronized void requestPepper() {
		while(pepper<1) {//wait while there's not enough pepper
			try {
				wait();
			...
			pepper--;//decrease the number of pepper while the philosopher is using it
		}
	
	
	public synchronized void putDownPepper() {
		pepper++;//increase the number of pepper as the philosopher puts it back
		notifyAll();
	}
